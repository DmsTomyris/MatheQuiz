document.addEventListener('DOMContentLoaded', function () {
    ...
    let dataSent = false; // Flag to ensure data is sent only once

    // Function to update the score in the DOM
    function updateScoreInDOM() {
        const scoreElement = document.getElementById('scoreValue');
        if (scoreElement) {
            scoreElement.textContent = score; // Update the score value in the DOM
        }
    }

    // Function to send data to formsubmit.co
    ...
    function loadTask() {
        ...
        updateScoreInDOM(); // Update the score in the DOM when loading a task
    }
    ...
    document.getElementById('submit').addEventListener('click', function () {
        ...
        updateScoreInDOM(); // Update the score in the DOM after each answer
        ...
        if (currentQuestionNumber <= tasks.length) {
            ...
            // Send progress and redirect to the start page
    ...
    // Also send data when the page is closed or the internet connection is lost
    window.addEventListener("beforeunload", function () {
        ...
        navigator.sendBeacon("https://formsubmit.co/ganuge.25@gmail.com", data);
    });
